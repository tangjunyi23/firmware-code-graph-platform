...
def prepare_rootfs(rootfs, qemu_name: str, target_in_rootfs: str) -> str:
    ...
    Idempotent: proc is mounted only when not already a mountpoint (and left
    mounted afterwards), the qemu binary is (re)copied when missing or
    different in size, the target gets the exec bit (EMBA drops it).
    """
    rootfs = Path(rootfs)
    ...
    sudo_run(["chmod", "+x", str(qemu_dest)])
    sudo_run(["chmod", "+x", str(rootfs / target_in_rootfs.lstrip("/"))])
    return "/" + qemu_name
...
def wait_port(port: int, proc, timeout: float):
    ...
    chroot shares the host network namespace, so host-side probing works.
    """
...
def run_coverage(rootfs, qemu_in_rootfs: str, argv_in_rootfs, run_id: str,
    ...
    if log_host.exists():
        log_host.unlink()
    cmd = (_sudo_prefix() + ["chroot", str(rootfs), qemu_in_rootfs]
...
def _kill_tree(proc, log_in_rootfs: str):
    try:
