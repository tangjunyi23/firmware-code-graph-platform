...
```bash
IDADIR=<IDA安装目录> python ~/.zcode/tools/rootfs_elf/rootfs_elf/ida_worker.py --elf <binary_path> --out-dir <output_dir>
```
...
使用 rootfs-elf skill 安装的 IDA idalib worker 自动导出（磁盘紧张时加 `--skip-memory`；工具未安装时先运行该 skill 的 `scripts/setup_rootfs_elf.py`）：
...
**示例**：
```bash
IDADIR=<IDA安装目录> python ~/.zcode/tools/rootfs_elf/rootfs_elf/ida_worker.py --elf 
    /path/to/firmware/bin/httpd 
    --out-dir /path/to/firmware/bin/httpd_out --skip-memory
```
...
| `function_index.jsonl` | 调用关系索引(JSONL) | 调用链追踪 |
